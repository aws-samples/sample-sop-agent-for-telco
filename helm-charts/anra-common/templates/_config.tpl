...
version: "1"
agent_role: {{ .Values.agentRole | default "anra" | quote }}
...
monitoring:
  ...
  influxdb_token_secret: {{ (.Values.monitoring).influxdbTokenSecret | default "anra-secrets" | quote }}
...
topology:
  ...
  emit_service_topology: {{ if hasKey ((.Values.config).topology | default dict) "emitServiceTopology" }}{{ .Values.config.topology.emitServiceTopology }}{{ else }}true{{ end }}
